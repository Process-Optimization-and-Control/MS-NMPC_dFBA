...
nc = 5 # number of ODEs
nfe    =  300      # number of control intervals
...
u[2,nfe-step_index+1:end]=klos*u[2,nfe-step_index+1:end]
       ...
       u_ini, x_ini, v_ini, solf, soltime, solvar = pFBA_KKT_flux(x0,x0,0.1,v0_ini,u_n,nfe,th)
    ...
    for i in 1:nfe
      ...
      #  FVA_ind = "min"
          ...
          # Generating data for plotting
           ...
           u_n =  u[1,i]
            println(i)
           solu, solx, solv, solf, soltime, solvar = pFBA_KKT_flux(xk1[:,i+1], x_ini[:,i:end,:],u_ini[:,i:end],v_ini[:,i:end],u_n,nfe_n,th_n)
            u[1,i+1] = solu[1,1]
            flags[i] = string(solf)
            time_opt[i] = soltime
            var_opt[i] = solvar
            ...
            #println(i)
            x_ini[:,i+1:end,:] = solx[:,2:end,:]
            v_ini[:,i+1:end] = solv[:,2:end]
            u_ini[i+1:end] = solu[2:end]
