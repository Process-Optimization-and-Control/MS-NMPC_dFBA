...
Si = readdlm("Si.csv", ',');
#E.coli iJR904
iobj=269  #objective function index
iglu=398  #glucose uptake
io2=500  #oxygen uptake
iac=289
iATP=212
...
vlb = readdlm("lb.csv", ',');
vub = readdlm("ub.csv", ',');
vlbi = readdlm("lbi.csv", ',');
vubi = readdlm("ubi.csv", ',');
nc = 5
...
vlb_Fi = deepcopy(vlbi)
vub_Fi = deepcopy(vubi)
...
w[ac,1] = -1e1  # max -1e-4
w[ac,2] = 1e2   # min 2.55e-4 
phi = 1.0
phi_all = 1e-4
phi1=phi_all
phi2=phi_all
phi3=phi_all
phi4 =  1e-2  #1e-4  # slacks
