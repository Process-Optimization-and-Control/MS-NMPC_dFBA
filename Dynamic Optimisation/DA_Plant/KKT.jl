function KKT(vg_k,vo_k,w)
   ...
   # m = Model(with_optimizer(Ipopt.Optimizer, warm_start_init_point = "yes", print_level = 5, linear_solver = "ma57") )
    ...
    set_attribute(m,"linear_solver", "ma57")
   ...
   #@objective(m, Min, 1)
    ...
    Lagr[mc=1:nv], +d[mc] + w[mc]*v[mc] + alpha_L[mc] + alpha_U[mc] + sum(S[k,mc]*lambda[k] for k in 1:nm) == 0
    ...
    #flag = is_solved_and_feasible(m)
    flag = termination_status(m)
    #vStar = JuMP.value.(v[:])
