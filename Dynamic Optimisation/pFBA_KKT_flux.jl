function pFBA_KKT_flux(c0,c_ini,v0)
   ...
   for i in 1:nc
    ...
    vg[i=1:nfe], vg_max*( (cs[1]*c[1,i,3])/(Kg+(cs[1]*c[1,i,3]) + ((cs[1]*c[1,i,3])^2/Ki) ))
    vo[i=1:nfe], vo_max* ( (cs[4]*c[4,i,3])/(Ko + (cs[4]*c[4,i,3])))
    
    end)
  
  @objective(m, Min, -phi*c[3,nfe,3] + sum( sum(-phi1*FO_L[mc,i] + -phi3*FO_U[mc,i] for mc in 1:nv) + sum(phi2*FO_upt[i,j] for j in 1:2) for i in 1:nfe))
  ...
  #@objective(m, Min, 1)
    ...
    @constraints(m, begin
        ...
        m1[i=1:nfe, j=1:ncp], cdot[1,i,j] == (0.18*vs[glu]*v[glu,i]*c[3,i,j]*cs[3] + (Sf - c[1,i,j]*cs[1])*(u[1,i]/c[5,i,j]*cs[5]))/cs[1]
        m2[i=1:nfe, j=1:ncp], cdot[2,i,j] == ( 0.06*vs[ac]*v[ac,i]*c[3,i,j]*cs[3] - c[2,i,j]*cs[2]*(u[1,i]/(c[5,i,j]*cs[5])))/cs[2]
        m3[i=1:nfe, j=1:ncp], cdot[3,i,j] == vs[obj]*v[obj,i]*c[3,i,j] - c[3,i,j]*(u[1,i]/c[5,i,j])
        m4[i=1:nfe, j=1:ncp], cdot[4,i,j] ==  (vs[o2]*v[o2,i]*c[4,i,j]*cs[4]*c[5,i,j]*cs[5] + klo*(O_sat - c[4,i,j]*cs[4]))/cs[4]
        m5[i=1:nfe, j=1:ncp], cdot[5,i,j] ==  u[1,i]
        ...
        #Process constriants
        V_UB[mc=5:5,i=1:nfe, j=1:ncp], c[mc,i,j]  <= V_max
        Ac_UB[mc=2:2,i=1:nfe, j=1:ncp], c[mc,i,j]  <= Ac_max
        u_LB[i=1:nfe], -u[1,i]  <= u_min
        u_UB[i=1:nfe],  u[1,i]  <= u_max
        u_rate01,  u[1,1] - u_ini  <= u_max_rate
        u_rate02,  u[1,1] - u_ini  >= -u_max_rate
        u_rate_UB[i=2:nfe],  u[1,i] - u[1,i-1]  <= u_max_rate
        u_rate_LB[i=2:nfe],  u[1,i] - u[1,i-1]  >= -u_max_rate
        ...
        #pFBA
        # Lagr[mc=1:nv,i=1:nfe], +d[mc] + w[mc]*v[mc,i]*vs[mc]  + alpha_L[mc,i] + alpha_U[mc,i] + up[mc]*alpha_upt[i] + sum(S[k,mc]*lambda[k,i] for k in 1:nm) == 0
        ...
        alpha4_LB[i=1:nfe,j=1:2], alpha_upt[i,j] <= 0
