...
ts = pushfirst!(ts,0)
    ...
    #v_ini = solv
         ...
         solu, sol, solv, solh = pFBA_KKT_flux(x0,x_ini,0.1,v_ini,u0,nfe,th)
...
p11 = plot(tsn,xk[:,3],linewidth=2,xaxis="time [h]",yaxis="biomass [g/L]",legend=false)
...
p12 = plot(tsn,xk[:,2],linewidth=2,xaxis="time [h]",yaxis="Ac [g/L]",legend=false)
...
p14 = plot(tsn,xk[:,1],linewidth=2,xaxis="time [h]",yaxis="Glucose [g/L]",legend=false)
...
p13 = plot(tsn,xk[:,4],linewidth=2,xaxis="time [h]",yaxis="Oxygen [mmol/L]",legend=false)
...
# p22 = plot(tsn,u[2,:],linewidth=2,xaxis="time [h]",yaxis="Klo [1/h]",legend=false)
 p23 = plot(tsn,xk[:,5],linewidth=2,xaxis="time [h]",yaxis="Volume [L]",legend=false)
 g2 = plot(p21,p23,layout=(2,1),reuse = true)

p31 = plot(ts,vk[:,obj],linewidth=2,xaxis="time [h]",yaxis="Growth rate [1/h]",legend=false)
...
p32 = plot(ts,vk[:,ac],linewidth=2,xaxis="time [h]",yaxis="Acetate [mmol/gDW L]",legend=false)
...
p33 = plot(ts,vk[:,glu],linewidth=2,xaxis="time [h]",yaxis="Glucose [mmol/gDW L]",legend=false)
...
p34 = plot(ts,vk[:,o2],linewidth=2,xaxis="time [h]",yaxis="Oxygen [mmol/gDW L]",legend=false)
